...
def make_sample_df(n=100):
    ...
    data = {"timestamp": dates, "value": np.arange(n)}
    return pd.DataFrame(data)


def test_split_random():
    df = make_sample_df(50)
    train, val, test = DataSplitter.split_random(
...
def test_split_time_series():
    ...
    train, val, test = DataSplitter.split_time_series(
        df, train_ratio=0.5, val_ratio=0.25
    )
    assert len(train) + len(val) + len(test) == len(df)
...
def test_split_by_time():
    ...
    train, val, test = DataSplitter.split_by_time(
        df, train_end="2023-01-10", val_end="2023-01-20"
    )
    assert len(train) + len(val) + len(test) == len(df)
...
def test_create_sliding_windows():
    ...
    X, y = DataSplitter.create_sliding_windows(
        arr, window_size=5, target_size=2, stride=1
    )
