...
class TestDataStorageManager:
    ...
    def test_retrieve_data(self, storage_manager, sample_data):
        ...
        try:
            ...
            logger.debug("Sample data:\n{}".format(sample_reset.head()))
            logger.debug("Retrieved data:\n{}".format(retrieved_reset.head()))
            ...
            for col in sample_reset.columns:
                if not (sample_reset[col] == retrieved_reset[col]).all():
                    ...
                    logger.error("Sample values: {}".format(sample_reset[col].head()))
                    logger.error("Retrieved values: {}".format(retrieved_reset[col].head()))
            raise
    ...
    def test_restore_backup(self, storage_manager, sample_file, test_dir):
        ...
        success = storage_manager.restore_backup(
    ...
    def test_cleanup_old_backups(self, storage_manager, sample_file, monkeypatch):
        ...
        for i in range(10):
            ...
            storage_manager.metadata[backup_timestamp] = storage_manager.metadata.pop(backup_info['timestamp'])
        ...
        assert len(storage_manager.metadata) >= 5, f"Too few backups kept: {len(storage_manager.metadata)}"
